...
public interface AttendanceReviseRepository extends CrudRepository<AttendanceRevise, String> {
	...
	@Query(value = "SELECT (max(ar.reviseId)+1) FROM AttendanceRevise ar")
	public String maxReviseId();
	
	@Query(value = "SELECT attendance_revise.revise_id, "
			+ "DATE_FORMAT(attendance_revise.submit_date, \"%d-%m-%Y\"), "
			+ "attendance_revise.attendance_id, "
			+ "shift_allocation.department_name, "
			+ "attendance_revise.employee_id, "
			+ "shift_allocation.employee_name, "
			+ "attendance_revise.shift_id, "
			+ "shift_allocation.shift_name, "
			+ "DATE_FORMAT(attendance_revise.date, \"%d-%m-%Y\"), "
			+ "attendance_revise.on_time, "
			+ "attendance_revise.off_time, "
			+ "attendance_revise.new_on_time, "
			+ "attendance_revise.new_off_time, "
			+ "attendance_revise.remark, "
			+ "attendance_revise.approved\r\n" + 
			...
			+ " attendance_revise.shift_id = shift_allocation.shift_id", nativeQuery = true)
	...
	@Query(value = "SELECT attendance_revise.revise_id, DATE_FORMAT(attendance_revise.submit_date, \"%d-%m-%Y\"), attendance_revise.attendance_id, shift_allocation.department_name, attendance_revise.employee_id, shift_allocation.employee_name, attendance_revise.shift_id, shift_allocation.shift_name, \r\n" + 
			...
			"WHERE attendance_revise.submit_date BETWEEN :startDate AND :endDate", nativeQuery = true)
	public List<String> loadAttendanceRevisesByDate(@Param("startDate") String startDate, @Param("endDate") String endDate);
	...
	@Query(value = "SELECT attendance_revise.revise_id, DATE_FORMAT(attendance_revise.submit_date, \"%d-%m-%Y\"), attendance_revise.attendance_id, shift_allocation.department_name, attendance_revise.employee_id, shift_allocation.employee_name, attendance_revise.shift_id, shift_allocation.shift_name, \r\n" + 
			...
			"attendance_revise.employee_id = shift_allocation.employee_id AND\r\n" + 
			"attendance_revise.shift_id = shift_allocation.shift_id\r\n" + 
			"WHERE attendance_revise.submit_date BETWEEN :startDate AND :endDate AND attendance_revise.approved = :approvalStatus", nativeQuery = true)
	public List<String> loadAttendanceRevisesByApprovalStatus(@Param("startDate") String startDate, @Param("endDate") String endDate, 
