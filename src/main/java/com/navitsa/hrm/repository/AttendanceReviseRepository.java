...
public interface AttendanceReviseRepository extends CrudRepository<AttendanceRevise, String> {
	...
	@Query(value = "SELECT attendance_revise.revise_id, " + "DATE_FORMAT(attendance_revise.submit_date, \"%Y-%m-%d\"), "
			...
			+ "shift_allocation.shift_name, " + "DATE_FORMAT(attendance_revise.date, \"%Y-%m-%d\"), "
	...
	@Query(value = "SELECT attendance_revise.revise_id, DATE_FORMAT(attendance_revise.submit_date, \"%Y-%m-%d\"), attendance_revise.attendance_id, shift_allocation.department_name, attendance_revise.employee_id, shift_allocation.employee_name, attendance_revise.shift_id, shift_allocation.shift_name, \r\n"
			+ "DATE_FORMAT(attendance_revise.date, \"%Y-%m-%d\"), attendance_revise.on_time, attendance_revise.off_time, attendance_revise.new_on_time, attendance_revise.new_off_time, attendance_revise.remark, attendance_revise.approved, attendance_revise.company_id \r\n"
			+ "FROM attendance_revise \r\n"
			+ "INNER JOIN shift_allocation ON attendance_revise.date = shift_allocation.date AND\r\n"
